    ...
    // ==========================================
    // 📸 ROUTE: CAPTURE SCREENSHOT WITH METADATA AND CROP
    // ==========================================
    app.post("/capture-and-crop-screenshot", async (req, res) => {
        const debugLogs = [];
        const log = (msg, data = null) => {
            const timestamp = new Date().toISOString();
            const entry = { timestamp, msg, data };
            console.log(`[SCREENSHOT_CROP_DEBUG] ${msg}`, data ? JSON.stringify(data).substring(0, 300) + "..." : "");
        ...
        try {
            ...
            const { screenshot_base64, image_metadata } = req.body;
            ...
            log("SCREENSHOT_RECEIVED", { 
                ...
                imageMetadataCount: image_metadata ? image_metadata.length : 0
            });

            // Decode base64 to buffer and save screenshot
            ...
            const blobTextPath = path.join(__dirname, "uploads", `screenshot_${timestamp}_blob.txt`);
            const screenshotMetadataPath = path.join(__dirname, "uploads", `screenshot_${timestamp}_metadata.json`);
            ...
            try {
                ...
                log("SCREENSHOT_SAVE_ERROR", { error: writeError.message, path: screenshotPath });
                throw new Error(`Failed to save screenshot: ${writeError.message}`);
            }

            // Save blob data as TEXT FILE (base64)
            try {
                fs.writeFileSync(blobTextPath, screenshot_base64);
                log("BLOB_TEXT_SAVED", { 
                    path: blobTextPath,
                    size: screenshot_base64.length,
                    exists: fs.existsSync(blobTextPath)
                });
                console.log(`✅ Blob text saved to: ${blobTextPath}`);
            } catch (writeError) {
                log("BLOB_TEXT_SAVE_ERROR", { error: writeError.message });
                // Don't throw - blob text is optional
            }
            ...
            try {
                const metadata = {
                    ...
                    blob_text_path: blobTextPath,
                    timestamp: timestamp,
                    image_metadata: image_metadata || []
                };
                fs.writeFileSync(screenshotMetadataPath, JSON.stringify(metadata, null, 2));
                log("METADATA_SAVED", { 
                    path: screenshotMetadataPath,
                    exists: fs.existsSync(screenshotMetadataPath)
                });
                console.log(`✅ Metadata saved to: ${screenshotMetadataPath}`);
            } catch (writeError) {
                log("METADATA_SAVE_ERROR", { error: writeError.message, path: screenshotMetadataPath });
                throw new Error(`Failed to save metadata: ${writeError.message}`);
            }

            // Crop images if metadata is provided
            const cropResults = [];
            if (image_metadata && Array.isArray(image_metadata)) {
                const execPromise = promisify(exec);
                const pythonScript = path.join(__dirname, "crop_image.py");

                for (let i = 0; i < image_metadata.length; i++) {
                    const imgMeta = image_metadata[i];
                    const cropOutputPath = path.join(__dirname, "uploads", `cropped_${timestamp}_image_${i}_${imgMeta.nodeId}.png`);
                    
                    try {
                        // Get crop coordinates from position or boundsInParent
                        const cropX = imgMeta.position?.x || imgMeta.boundsInParent?.x || 0;
                        const cropY = imgMeta.position?.y || imgMeta.boundsInParent?.y || 0;
                        const cropWidth = imgMeta.position?.width || imgMeta.boundsInParent?.width || 0;
                        const cropHeight = imgMeta.position?.height || imgMeta.boundsInParent?.height || 0;

                        if (cropWidth > 0 && cropHeight > 0) {
                            log("CROPPING_IMAGE", { 
                                nodeId: imgMeta.nodeId,
                                x: cropX,
                                y: cropY,
                                width: cropWidth,
                                height: cropHeight
                            ...
                            // Call Python script to crop (using command-line arguments)
                            const x = Math.round(cropX);
                            const y = Math.round(cropY);
                            const w = Math.round(cropWidth);
                            const h = Math.round(cropHeight);
                            
                            const command = `python "${pythonScript}" "${screenshotPath}" "${cropOutputPath}" ${x} ${y} ${w} ${h}`;
                            
                            const { stdout, stderr } = await execPromise(command, {
                                maxBuffer: 10 * 1024 * 1024 // 10MB buffer
                            });

                            const cropResult = JSON.parse(stdout);
                            
                            if (cropResult.success) {
                                cropResults.push({
                                    nodeId: imgMeta.nodeId,
                                    original_path: screenshotPath,
                                    cropped_path: cropResult.output_path,
                                    crop_area: cropResult.crop_area,
                                    success: true
                                });
                                log("IMAGE_CROPPED_SUCCESS", { nodeId: imgMeta.nodeId, path: cropResult.output_path });
                            } else {
                                cropResults.push({
                                    nodeId: imgMeta.nodeId,
                                    success: false,
                                ...
                                log("IMAGE_CROP_FAILED", { nodeId: imgMeta.nodeId, error: cropResult.error });
                            }
                        } else {
                            log("SKIP_CROP_INVALID_DIMENSIONS", { nodeId: imgMeta.nodeId, width: cropWidth, height: cropHeight });
                        }
                    } catch (cropError) {
                        log("CROP_ERROR", { nodeId: imgMeta.nodeId, error: cropError.message });
                        cropResults.push({
                            nodeId: imgMeta.nodeId,
                            success: false,
            ...
            const metadataExists = fs.existsSync(screenshotMetadataPath);
            
            log("FILES_VERIFICATION", {
                screenshot_exists: screenshotExists,
                metadata_exists: metadataExists,
                screenshot_path: screenshotPath,
                metadata_path: screenshotMetadataPath
            });

            if (!screenshotExists || !metadataExists) {
                throw new Error(`Files not saved properly. Screenshot: ${screenshotExists}, Metadata: ${metadataExists}`);
            }

            const screenshotSize = fs.statSync(screenshotPath).size;
            const metadataSize = fs.statSync(screenshotMetadataPath).size;
            const blobTextExists = fs.existsSync(blobTextPath);
            const blobTextSize = blobTextExists ? fs.statSync(blobTextPath).size : 0;
            
            console.log(`\n✅✅✅ FILES SAVED TO UPLOADS FOLDER ✅✅✅`);
            console.log(`📸 Screenshot PNG: ${screenshotPath} (${(screenshotSize / 1024).toFixed(2)} KB)`);
            if (blobTextExists) {
                console.log(`📄 Blob Text File: ${blobTextPath} (${(blobTextSize / 1024).toFixed(2)} KB)`);
            }
            console.log(`📋 Metadata JSON: ${screenshotMetadataPath} (${(metadataSize / 1024).toFixed(2)} KB)`);
            if (cropResults.length > 0) {
                console.log(`✂️  Cropped images: ${cropResults.filter(r => r.success).length} file(s)`);
            }
            console.log(`\n`);
            ...
            res.json({
                success: true,
                screenshot_path: screenshotPath,
                blob_text_path: blobTextExists ? blobTextPath : null,
                metadata_path: screenshotMetadataPath,
                screenshot_size: screenshotSize,
                blob_text_size: blobTextSize,
                metadata_size: metadataSize,
                screenshot_base64: screenshot_base64.substring(0, 100) + "...", // Truncated for response
                image_metadata: image_metadata || [],
                crop_results: cropResults,
                debugLogs,
                message: `Files saved successfully to uploads folder: screenshot PNG, blob text file, metadata JSON`
            });
